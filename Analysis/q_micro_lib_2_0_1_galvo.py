...
def yValueFunc(ampY, yfreq, time): # Returns the function for the Y value for the swipe function (unit ampere)
    return ampY * np.sin(2 * np.pi * yfreq * time - (np.pi / 2)) #- 0.289 #Measurement should start at peak or valley so it should be phase pi/2 
...
def createXYMatrix(dimX, dimY, pixelAY, ampY, xvaluelist, yvaluelist, countrate, time_axis):
    ...
    yRail = []
    for i in range(dimY): #Create a list with total current over the full intervall (2 * amp) divided into each pixel segment. This will be used to make decision which pixel bin to place the counts
