        ...
        while True:
            ...
            if len(full_msg) - HEADERSIZE == msglen:
                ...
                if type(msg_sent) == 'dict':     # checking if the message is in the proper form
                    ...
                    msg_sent = pickle.loads(full_msg[HEADERSIZE:])
                    file = msg_sent['file']
                    scantime = int(msg_sent['scantime'])
                    new_msg = True
                    full_msg = b''
...
def main():
    ...
    (file,scan_time) = socket_connection()  #connects to the server and receive the data
    dummy_function(file, scantime)
    #run_qutag(file,scan_time)       #starts the measurement
